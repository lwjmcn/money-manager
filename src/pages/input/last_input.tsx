...
const LastInput = ({ navigation }) => {
  const { isDark } = useTheme();
  const [nextDisabled, setNextDisabled] = useState<boolean>(true);
  const handleNext = () => {
    navigation.navigate(InputTabs.DONE);
  };
  return (
    <InputLayout navigation={navigation} title="새로운 내역 입력하기" step={4} onNextPress={handleNext} nextDisabled={nextDisabled}>
      <Text>Second</Text>
    </InputLayout>
  );
};
